...
public class AddJuego extends javax.swing.JFrame {
	...
	public void checkFiles(String extension, File f) {
		...
		if (extension.equals("exe") || extension.equals("EXE")) {
			ImageIcon validate = new ImageIcon(getClass().getResource("/img/validate.png"));
			dragZone.setIcon(validate);
			btnAceptPath.setText("Aceptar");
			_urlJuego = f.toString();
			_urlJuego = _urlJuego.replace('\\', '/');
			ImageIcon icon = new ImageIcon(getClass().getResource("/img/Aceptar.png"));
			btnAceptPath.setIcon(icon);
			btnAceptPath.setActionCommand("Validar");
		} else {
			btnAceptPath.setText("Ejecutable no Encontrado");
			ImageIcon validate = new ImageIcon(getClass().getResource("/img/notExe.png"));
			dragZone.setIcon(validate);
			ImageIcon icon = new ImageIcon(getClass().getResource("/img/cross.png"));
			btnAceptPath.setIcon(icon);
		}
	}
		...
		TransferHandler th = new TransferHandler() {
			...
			public boolean importData(JComponent comp, Transferable t) {
				try {
					...
					if (files.size() == 1) {
						File f = files.get(0);
						String extension = getFileExtension(f);//Obtenemos la extension del fichero
						checkFiles(extension, f);
					}
